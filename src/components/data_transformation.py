...
class DataTransformation:
    ...
    def initiate_data_transformation(self,train_path,test_path):
        ...
        try:
            ...
            target_feature_train_df=target_feature_train_df.apply(lambda x: 1 if x=='Y'  else 0)


            input_feature_test_df=test_df.drop(columns=[target_column_name,"Loan_ID"],axis=1)
            target_feature_test_df=test_df[target_column_name]
            target_feature_test_df=target_feature_test_df.apply(lambda x: 1 if x=='Y'  else 0)
            logging.info(
